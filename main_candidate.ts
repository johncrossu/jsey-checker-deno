...
async function generatePdfReport(data: any, reportType: string): Promise<Uint8Array> {
  ...
  if (reportType === "token") {
    ...
    doc.text(`Address: ${data.address || "N/A"}`, leftMargin + 12, boxTop1 + 10, { width: contentWidth - 24 });
    doc.text(`Chain: ${data.chain || "N/A"}`, leftMargin + 12, boxTop1 + 30);
    doc.text(`Name: ${data.tokenName || "N/A"}`, leftMargin + 12, boxTop1 + 46);
    doc.text(`Symbol: ${data.tokenSymbol || "N/A"}`, leftMargin + 12, boxTop1 + 62);
    doc.y = boxTop1 + 78 + 16;
    doc.x = leftMargin;
    ...
    if (data.summary) {
      doc.fontSize(9).fillColor("#555").font("Inter-Italic").text(data.summary, leftMargin, doc.y, { width: contentWidth });
      doc.moveDown(0.5);
    }
    ...
    if (data.reasons && data.reasons.length) {
      ...
      data.reasons.forEach((r: string) => {
        doc.fontSize(9).text(`- ${r}`, leftMargin + 10, doc.y, { width: contentWidth - 20 });
      });
      doc.moveDown(0.4);
    }
    ...
    if (data.deployerData && data.deployerData.available) {
      ...
      doc.text(`Wallet: ${data.deployerData.deployerAddress || "N/A"}`, leftMargin + 10, doc.y);
    ...
    doc.fontSize(8).fillColor(GRAY).font("Inter").text("jsey.dpdns.org  |  support@jsey.dpdns.org", leftMargin, legalTop + 78, { width: contentWidth, align: "center" });
    ...
    const legalTop = y;
    doc.rect(leftMargin, legalTop, contentWidth, 70).strokeColor(LIGHT_BORDER).lineWidth(1).stroke();
    doc.font("Inter-Italic").fontSize(7.5).fillColor("#555").text(
      "This report reflects blockchain data available at the time of scan. J-SEY makes no claims of one hundred percent scam or honeypot detection accuracy, and this report does not constitute financial, legal, or investment advice. It is not a guarantee of safety, legitimacy, or profitability of any token or wallet referenced herein. Users act at their own risk and should conduct independent due diligence before any transaction.",
      leftMargin + 12, legalTop + 10, { width: contentWidth - 24 }
    );
    ...
    if (riskyTokens.length) {
      ...
      for (const [chainName, tokens] of groups) {
        ...
        firstGroup = false;
        doc.font("Inter-Bold").fontSize(13).fillColor(NAVY).text(`${chainName} — Risky Tokens (${tokens.length})`, leftMargin, doc.y);
        doc.moveDown(0.4);
        tokens.forEach((t: any) => {
          if (doc.y > pageHeight - 100) doc.addPage();
          const rColor = t.riskLevel === "HIGH" ? RED : t.riskLevel === "MEDIUM" ? GOLD : GREEN;
          doc.x = leftMargin;
          doc.fontSize(10).fillColor("#111").font("Inter-Bold").text(`${t.tokenName || t.token} (${t.tokenSymbol || ""}) `, leftMargin, doc.y, { continued: true });
          doc.fillColor(rColor).text(t.riskLevel);
          doc.font("Inter").fillColor("#555");
          if (t.reasons) t.reasons.forEach((r: string) => {
            doc.fontSize(9).text(`- ${r}`, indentX, doc.y, { width: pageWidth - indentX - 50 });
          });
          doc.x = leftMargin;
          doc.moveDown(0.3);
        });
